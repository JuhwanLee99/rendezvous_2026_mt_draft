...
`firstPick`이 A일 때: **1R [청,백] · 2R [백,청] · 3R [청,백] …**
전 선수가 소진될 때까지 진행하며, 홀수면 마지막 픽 팀이 1명 더 가져간다.
모든 턴 상태는 단일 카운터 `pickCount`에서 파생되어 일관성이 보장된다
(엔진: [`src/lib/draftEngine.js`](src/lib/draftEngine.js)).

## 보안 모델 (무료 플랜의 한계 — 동아리 내부용 수용)

Firestore 보안 규칙이 강제하는 것:
- 관리자(하드코딩 UID)만 설정/풀/핀/시작/리셋/undo/대리지명
- 픽은 (a) 선수 미지명 (b) **현재 차례 팀**(스네이크 공식 규칙 인라인) (c) **순번 일치**일 때만 — 더블픽·순번 위조·차례 위조 차단
